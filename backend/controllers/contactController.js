...
const getContacts = asyncHandler(async (req, res) => {
  const { search, type, folder, source, stage, assignedTo, page = 1, limit = 20 } = req.query;
...
const mergeContact = asyncHandler(async (req, res) => {
  ...
  if (req.body.note) {
    contact.notes.push({ content: req.body.note, type: 'text', createdBy: req.user.id })
  }
...
const updateContact = asyncHandler(async (req, res) => {
  let contact = await Contact.findOne({ _id: req.params.id, owner: req.user.id })
...
const addReminder = asyncHandler(async (req, res) => {
  ...
  res.status(201).json({ success: true, reminder: newReminder, reminders: contact.reminders })
})
...
const getAllReminders = asyncHandler(async (req, res) => {
  ...
  const contacts = await Contact.find({ owner: userId, 'reminders.0': { $exists: true } })
  ...
  if (filter === 'today') {
    flattened = flattened.filter((r) =>
      r.status === 'pending' && new Date(r.dueDate) >= startOfDay && new Date(r.dueDate) <= endOfDay
    )
