...
function formatDueDisplay(task) {
  ...
  const timeLabel = task.dueTime
    ? (() => {
        ...
        d.setHours(h, m);
        return d
          .toLocaleTimeString('en-US', { hour: 'numeric', minute: m ? '2-digit' : undefined })
          .toLowerCase()
          .replace(' ', '');
      })()
    : null;
...
function TaskCard({ task, onOpen }) {
  ...
  return (
    <button
      onClick={onOpen}
      className={`w-full bg-white rounded-2xl px-4 py-4 text-left active:scale-[0.99] transition-transform ${CARD_SHADOW}`}
    >
      <div className="flex items-start gap-3.5">
        <div className="w-11 h-11 rounded-full bg-sage ring-1 ring-sage/20 flex items-center justify-center text-white font-bold text-[13px] shrink-0">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-2">
            <span className="inline-block px-2.5 py-1 rounded-full bg-forest/8 text-forest text-[11.5px] font-medium">
              {task.title}
            </span>
            <span className={`text-[11.5px] ${isOverdue ? 'text-red-500 font-medium' : 'text-gray-400'}`}>
...
function TasksSkeleton() {
  return (
    <div className="flex flex-col gap-3 pt-1">
      {[...Array(5)].map((_, i) => (
        <div key={i} className="h-[80px] rounded-2xl bg-white/70 animate-pulse" />
      ))}
    </div>
  );
}
...
export default function Tasks() {
  ...
  useEffect(() => {
    ...
    async function fetchTasks() {
      ...
      try {
        ...
        if (sortFilters.relationshipType) params.relationshipType = sortFilters.relationshipType;
  ...
  }, [scope, quickFilter, sortFilters, isFounder, user?._id]);
  ...
  return (
    <div className="max-w-[480px] mx-auto min-h-screen bg-bg flex flex-col">
      ...
      {/* Header */}
      <div className="bg-gradient-to-br from-sage to-forest px-5 pt-5 pb-6 shrink-0 rounded-b-[28px] shadow-[0_8px_24px_-8px_rgba(0,0,0,0.15)]">
        <div className="flex items-center justify-between">
          ...
          <span className="text-white font-semibold text-[16px]">Tasks</span>
          <div className="w-9 h-9" />
        </div>

        {isFounder && (
          <div className="flex gap-1 bg-white/15 rounded-full p-1 mt-5">
            <button
              onClick={() => setScope('team')}
              className={`flex-1 h-9 rounded-full text-[13px] font-semibold transition-colors ${
                scope === 'team' ? 'bg-white text-forest shadow-sm' : 'text-white/80'
              }`}
            >
              Team Tasks
            </button>
            <button
              onClick={() => setScope('my')}
              className={`flex-1 h-9 rounded-full text-[13px] font-semibold transition-colors ${
                scope === 'my' ? 'bg-white text-forest shadow-sm' : 'text-white/80'
              }`}
            >
              My Tasks
            </button>
      ...
      {/* Filters */}
      <div className="flex items-center gap-2 overflow-x-auto px-5 pt-4 pb-1 shrink-0 -mt-1">
        <button
          onClick={() => navigate('/tasks/sort', { state: { filters: sortFilters } })}
          className={`w-9 h-9 rounded-full bg-white flex items-center justify-center shrink-0 ${CARD_SHADOW}`}
        >
        ...
        {QUICK_FILTERS.map((f) => (
          <button
            ...
            className={`h-9 px-4 rounded-full text-[13px] font-medium whitespace-nowrap shrink-0 transition-colors ${
              quickFilter === f ? 'bg-forest text-white' : `bg-white text-gray-500 ${CARD_SHADOW}`
            }`}
          >
            {f}
          </button>
      ...
      {/* Task list */}
      <div className="flex-1 px-5 pt-3 pb-28 overflow-y-auto">
        {loading ? (
          <TasksSkeleton />
        ) : tasks.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center gap-3">
            <div className="w-16 h-16 rounded-full bg-sage/12 flex items-center justify-center">
              <img src="/assets/icons/tab-followup.svg" alt="" className="w-6 h-6 opacity-40" />
            </div>
            <p className="text-[14px] font-semibold text-gray-700">No tasks found</p>
      ...
      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[480px] bg-forest relative px-2 pt-3 pb-2 shrink-0">
        <div className="flex items-center justify-between px-2">
          <button onClick={() => navigate('/home')} className="flex flex-col items-center gap-1 flex-1 py-1">
            <img src="/assets/icons/tab-home.svg" alt="" className="w-6 h-6 opacity-80" />
      ...
      {isFounder && (
        <button
          ...
          className="fixed bottom-24 right-5 w-12 h-12 rounded-full bg-forest text-white flex items-center justify-center shadow-[0_8px_20px_-4px_rgba(0,0,0,0.3)] text-2xl font-bold z-20 active:scale-95 transition-transform"
        >
          +
        </button>
      )}
