...
const TASK_STATS = [
  ...
  },
  {
    key: 'today',
    label: 'Due today',
    icon: '/assets/icons/due-today.svg',
    bg: 'bg-forest/10',
  },
  {
    key: 'upcoming',
    label: 'Upcoming',
    icon: '/assets/icons/upcoming.svg',
    bg: 'bg-sage/15',
  },
];
...
function StatBlock({ stat, value }) {
  return (
    <div className='flex-1 flex flex-col items-center gap-2'>
      ...
      <span className='font-bold text-[19px] text-gray-900 leading-none'>
        {value}
      </span>
...
function DrawerItem({ icon, label, onClick }) {
  return (
    <button
      onClick={onClick}
      className='w-full flex items-center gap-3.5 px-3 py-3 rounded-xl text-left active:bg-sage/5 transition-colors'
    >
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    async function loadDashboard() {
      ...
      try {
        const params =
          isFounder && homeScope === 'mine' ? { scope: 'mine' } : {};
              ...
              <div className='flex items-start mb-5'>
                {TASK_STATS.map((stat, i) => (
                  <div
                    ...
                    className={
                      i === 1
                        ? 'flex-1 border-x border-gray-100 px-2'
                        : 'flex-1 px-2'
                    }
                  >
                    <StatBlock
                      stat={stat}
                      value={dashboard?.tasks?.[stat.key] ?? 0}
                    />
                  </div>
                ))}
              </div>
              ...
              >
                ...
                <div className='flex-1 text-left'>
                  ...
                  <p className='text-[12px] text-gray-500 mt-0.5'>
                    {dashboard?.unassignedContactsCount ?? 0} awaiting
                    assignment
                  </p>
                </div>
                <img
