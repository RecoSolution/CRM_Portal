...
function QuickAction({ href, icon, label }) {
  const cls = `flex-1 flex flex-col items-center gap-1.5 bg-white rounded-2xl py-4 active:scale-[0.97] transition-transform ${CARD_SHADOW}`;
  return (
...
function DetailSkeleton() {
  return (
    <div className="flex-1 px-5 pt-6 pb-12 flex flex-col gap-7">
      <div className="flex items-center gap-4">
        <div className="w-16 h-16 rounded-full bg-white/70 animate-pulse shrink-0" />
        <div className="flex-1 flex flex-col gap-2">
          <div className="h-4 w-2/3 rounded-full bg-white/70 animate-pulse" />
          <div className="h-3 w-1/3 rounded-full bg-white/70 animate-pulse" />
        </div>
      </div>
      <div className="flex gap-2.5">
        {[...Array(3)].map((_, i) => <div key={i} className="flex-1 h-[76px] rounded-2xl bg-white/70 animate-pulse" />)}
      </div>
      <div className="h-[140px] rounded-2xl bg-white/70 animate-pulse" />
      <div className="h-[110px] rounded-2xl bg-white/70 animate-pulse" />
    </div>
  );
}
...
export default function ContactDetail() {
  ...
  return (
    <div className="max-w-[480px] mx-auto min-h-screen bg-bg flex flex-col relative">
      ...
      {/* Header */}
      <div className="bg-gradient-to-br from-sage to-forest flex items-center justify-between px-5 h-16 shrink-0 rounded-b-[28px] shadow-[0_8px_24px_-8px_rgba(0,0,0,0.15)] sticky top-0 z-30">
        ...
        <button onClick={() => setMenuOpen(!menuOpen)} className="w-9 h-9 flex items-center justify-center rounded-full active:bg-white/10 transition-colors">
          <img src="/assets/icons/more-vertical.svg" alt="menu" className="w-5 h-5 brightness-0 invert" />
        </button>
      </div>
      ...
      {menuOpen && (
        <>
          ...
          <div className={`absolute right-5 top-[72px] z-50 bg-white rounded-2xl py-1.5 w-48 overflow-hidden ${CARD_SHADOW} shadow-[0_12px_32px_-8px_rgba(0,0,0,0.18)]`}>
        ...
        <div className="flex-1 px-5 pt-6 pb-12 flex flex-col gap-7">
            ...
            <div>
              <div className="flex items-center justify-between mb-2.5 px-1">
                ...
                <button
                  ...
                  className="h-7 px-3.5 rounded-full bg-forest text-white text-[11.5px] font-semibold active:scale-95 transition-transform"
                >
                  Mark Complete
                </button>
              </div>
              <div className={`flex items-center justify-between bg-white rounded-2xl px-4 py-3.5 ${CARD_SHADOW}`}>
                <div className="flex items-center gap-2.5 min-w-0">
                  <span className="w-2 h-2 rounded-full bg-amber-500 shrink-0" />
                  <span className="text-[13px] text-gray-800 capitalize truncate">
          ...
          {!showReminderForm ? (
            <button
              ...
              className="w-full h-12 rounded-2xl border-[1.5px] border-dashed border-forest/40 text-forest text-[13.5px] font-semibold active:bg-forest/5 transition-colors"
            >
              + Add Reminder
            </button>
          ) : (
            <div className={`bg-white rounded-2xl p-4 ${CARD_SHADOW}`}>
              ...
              <div className="flex gap-2 mb-4">
                {['low', 'medium', 'high'].map((p) => (
                  <button
                    ...
                    className={`flex-1 h-9 rounded-full text-[12.5px] font-semibold capitalize transition-colors ${
                      reminderPriority === p ? 'bg-forest text-white' : 'bg-sage/10 text-gray-600'
                    }`}
                  >
                    {p}
                  </button>
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 h-11 rounded-full border border-gray-200 text-gray-600 text-[13px] font-semibold active:bg-gray-50 transition-colors"
                >
                  Cancel
                </button>
                <button
          ...
          {contact.notes?.length > 0 && (
            <div>
              ...
              <div className={`bg-white rounded-2xl px-4 py-3.5 flex flex-col gap-2 ${CARD_SHADOW}`}>
                {contact.notes.map((n) => (
          ...
          {contact.cardImageUrl && (
            <div>
              ...
              <img src={contact.cardImageUrl} alt="Business card" className={`w-full rounded-2xl ${CARD_SHADOW}`} />
            </div>
          )}
          ...
          <div>
            <button onClick={() => setHistoryOpen(!historyOpen)} className={`w-full flex items-center justify-between bg-white rounded-2xl px-4 py-3.5 mb-2 ${CARD_SHADOW}`}>
              ...
              <img
                ...
                alt=""
                className={`w-4 h-4 opacity-50 transition-transform ${historyOpen ? 'rotate-180' : ''}`}
              />
            </button>
            ...
            {historyOpen && (
              <div className={`bg-white rounded-2xl overflow-hidden ${CARD_SHADOW}`}>
                {activityEvents.map((event, i) => (
                  <div key={i} className={`flex items-center justify-between px-4 py-3.5 text-[12.5px] ${i > 0 ? 'border-t border-sage/10' : ''}`}>
