...
const CARD_SHADOW =
  'shadow-[0_2px_12px_-4px_rgba(0,0,0,0.06)] ring-1 ring-black/[0.03]';
...
function ContactRow({ contact, onClick }) {
  const badgeClass =
    LEAD_BADGE_STYLES[contact.leadCategory] || 'bg-gray-100 text-gray-500';
  ...
  return (
    ...
    >
      ...
      {contact.leadCategory && (
        <span
          className={`text-[10.5px] font-semibold px-2.5 py-1 rounded-full shrink-0 capitalize ${badgeClass}`}
        >
          {contact.leadCategory}
        </span>
      )}
  ...
  return (
    <div className='flex flex-col gap-2.5 mt-1'>
      {[...Array(6)].map((_, i) => (
        <div
          key={i}
          className='h-[68px] rounded-2xl bg-white/70 animate-pulse'
        />
      ))}
    </div>
  );
...
export default function Contacts() {
  ...
  // activeFilter is now derived from advancedFilters.relationshipType so the
  // pill row and the advanced filter modal can never disagree with each other.
  const [activeFilter, setActiveFilter] = useState(
    () => location.state?.filters?.relationshipType || 'All',
  );
  ...
  const hasActiveFilters =
    activeFilter !== 'All' || Object.keys(advancedFilters).length > 0;
  ...
  useEffect(() => {
    ...
    async function fetchContacts() {
      ...
      try {
        ...
        // Single source of truth for the relationship-type filter — no
        // more silent override between pills and the advanced filter modal.
