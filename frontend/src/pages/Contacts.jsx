...
const [advancedFilters, setAdvancedFilters] = useState(location.state?.filters || {});
    ...
    <div className="max-w-[480px] mx-auto min-h-screen bg-bg flex flex-col">
      ...
      <div className="bg-sage px-5 pt-5 pb-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={() => navigate('/contacts/filters', { state: { filters: advancedFilters } })} className='w-9 h-9 flex items-center justify-center'>
  <img src='/assets/icons/filter.svg' alt='filter' className='w-5 h-5' />
        ...
        <div className="relative">
          <img src="/assets/icons/search.svg" alt="" className="absolute left-4 top-1/2 -translate-y-1/2 w-4.5 h-4.5 opacity-60" />
          <input
            value={search}
        ...
        {FILTERS.map((f) => (
          <button
            ...
            className={`h-9 px-4 rounded-full text-[13px] font-medium whitespace-nowrap ${
              activeFilter === f ? 'bg-forest text-white' : 'bg-white/70 text-gray-500'
            }`}
          >
            {f}
          </button>
      ...
      <div className="flex-1 px-5 pb-10 overflow-y-auto">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            <img src="/assets/illustrations/no-contacts.png" alt="" className="w-32 h-32 object-contain mb-4 opacity-70" />
            ...
            {contacts.map((contact) => (
              ...
              >
                ...
                {contact.leadCategory && (
                  <span className={`text-[11px] font-semibold px-2.5 py-1 rounded-full shrink-0 ${leadBadgeColor(contact.leadCategory)}`}>
                    {contact.leadCategory}
                  </span>
                )}
              </button>
